  ...
  return <main className="min-h-screen bg-[#f7f8fa] text-slate-950">
    <header className="border-b border-slate-200/80 bg-white">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6">
        ...
        <div className="flex items-center gap-2"><Link className={buttonVariants({ variant: "outline", size: "sm" })} href="/account/settings">Seller profile</Link><Link className={buttonVariants()} href="/"><Plus data-icon="inline-start" />New invoice</Link><SignOutButton /></div>
      </div>
    </header>
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,0.72fr)_minmax(0,1.28fr)]">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white shadow-sm" aria-label="Customer directory"><div className="border-b border-slate-200 p-4 sm:p-5"><label className="relative block"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} /><input className="field pl-9" placeholder="Search customers" value={query} onChange={(event) => setQuery(event.target.value)} /></label></div>{error && <p className="m-4 rounded-lg bg-rose-50 px-3 py-2 text-sm text-rose-700" role="alert">{error}</p>}{loading ? <p className="p-8 text-sm text-slate-500">Loading customers…</p> : customers.length === 0 ? <div className="p-8 text-center"><p className="font-medium">{query ? "No customers match your search" : "No customers yet"}</p><p className="mt-2 text-sm text-slate-500">Add a customer to reuse their details on future invoices.</p></div> : <div className="divide-y divide-slate-100">{customers.map((customer) => <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-start sm:justify-between sm:p-5" key={customer.id}><div className="min-w-0"><p className="font-semibold">{customer.companyName || customer.name}</p>{customer.companyName && <p className="text-sm text-slate-600">{customer.name}</p>}{(customer.email || customer.phone) && <p className="mt-1 text-xs text-slate-500">{[customer.email, customer.phone].filter(Boolean).join(" · ")}</p>}{customer.address && <p className="mt-1 whitespace-pre-line text-xs text-slate-500">{customer.address}</p>}</div><div className="flex shrink-0 gap-3"><button className="inline-flex items-center gap-1 text-xs font-semibold text-slate-700 hover:underline" type="button" onClick={() => edit(customer)}><Pencil size={13} />Edit</button><button className="inline-flex items-center gap-1 text-xs font-semibold text-rose-600 hover:underline" type="button" onClick={() => void remove(customer)}><Trash2 size={13} />Remove</button></div></div>)}</div>}</section>
      </div>
    </div>
  </main>
}
