...
function Field({ label, type, value, onChange, required }: { label: string; type: string; value: string; onChange: (value: string) => void; required?: boolean }) { const autocomplete = type === "email" ? "email" : label.startsWith("Confirm") ? "new-password" : type === "password" ? "new-password" : undefined; return <label className="block"><span className="mb-2 block text-sm font-medium text-slate-700">{label}{required && <span className="ml-1 text-rose-500">*</span>}</span><input className="field" type={type} value={value} onChange={(event) => onChange(event.target.value)} autoComplete={autocomplete} required={required} /></label> }
