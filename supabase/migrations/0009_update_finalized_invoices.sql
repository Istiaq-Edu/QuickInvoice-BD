...
begin
  ...
  v_discount_type := coalesce(v_canonical->>'discountType', 'none');
  if v_discount_type not in ('none', 'fixed', 'percentage') then
  ...
  set issue_date = p_issue_date,
      ...
      discount_type = v_discount_type::public.discount_type,
